...
class RequestWithTokenRefresh: ApiRequest {
    ...
    private func completeWithError(_ error: OAuth2Error) {
        ...
        (responseQueue ?? .main).async {
            switch completionHandler  {
            case .dataCompletionHandler(let handler):
                let result = Result<Data?, AFError>.failure(AFError.sessionTaskFailed(error: error))
                let dataResponse = AFDataResponse<Data?>(request: nil, response: nil, data: nil, metrics: nil, serializationDuration: 0, result: result)
                handler(dataResponse)
            case .downloadFileCompletionHandler(let handler):
                let result = Result<URL?, AFError>.failure(AFError.sessionTaskFailed(error: error))
                let downloadResponse = AFDownloadResponse<URL?>(request: nil, response: nil, fileURL: nil, resumeData: nil, metrics: nil, serializationDuration: 0, result: result)
